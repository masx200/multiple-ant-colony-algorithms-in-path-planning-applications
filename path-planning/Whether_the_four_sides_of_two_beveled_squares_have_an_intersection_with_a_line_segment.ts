...
export function Whether_the_four_sides_of_two_beveled_squares_have_an_intersection_with_a_line_segment(
    ...
    return [
        ...
        const four_edges = [
            [
                /* 忽略四个角的坐标 */
                [x - 0.5 * 0.95, y - 0.5 * 0.95], // 第一个顶点坐标
                [x + 0.5 * 0.95, y - 0.5 * 0.95], // 第二个顶点坐标
            ],
            [
                //长宽缩到95%
                [x + 0.5 * 0.95, y - 0.5 * 0.95], // 第三个顶点坐标
                [x + 0.5 * 0.95, y + 0.5 * 0.95], // 第四个顶点坐标
            ],
            [
                //处理斜45度的情况
                [x + 0.5 * 0.95, y + 0.5 * 0.95], // 第五个顶点坐标
                [x - 0.5 * 0.95, y + 0.5 * 0.95], // 第六个顶点坐标
            ],
            [
                [x - 0.5 * 0.95, y + 0.5 * 0.95], // 第七个顶点坐标
                [x - 0.5 * 0.95, y - 0.5 * 0.95], // 第八个顶点坐标
            ],
