...
/**
 * 判断从起点到终点是否可以通过直线路径到达
 * @param start 起点的坐标，格式为 [x, y]
 * @param end 终点的坐标，格式为 [x, y]
 * @param grid 地图信息，包括列数、行数、障碍物信息等
 * @returns 如果可以从起点到终点通过直线路径到达，则返回true；否则返回false
 */
export function canStraightReachOld(
    start: [number, number],
    end: [number, number],
    grid: GridMap,
): boolean {
